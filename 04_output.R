...
#Normalize Provincial and Raw
CurrMapP<-rast(file.path(ConnDir,paste0('BC_P_1tile/CurMapMean.tif')))
CurrMapR<-rast(file.path(ConnDir,paste0('BC_R_1tile/CurMapMean.tif')))

CurrMapNorm<-CurrMapP/CurrMapR
writeRaster(CurrMapNorm2, filename=file.path(ConnDir,paste0('BC_P_1tile/CurrMapNorm2.tif')), overwrite=TRUE)
#Tried the other way doesnt show much..
CurrMapNorm2<-CurrMapR/CurrMapP

#Log transform data to explore breaks
CurrMapLog<-exp(CurrMapP)
writeRaster(CurrMapLog, filename=file.path(ConnDir,paste0('BC_P_1tile/CurrMapLog.tif')), overwrite=TRUE)
#Normalize
CurrMapP_norm<-raster.transformation(CurrMapP, trans = "norm", smin = 0, smax = 255)
writeRaster(CurrMapP_norm, filename=file.path(ConnDir,paste0('BC_P_1tile/CurrMapP_norm.tif')), overwrite=TRUE)
#Natural Log
CurrMapP_natLog<-raster.transformation(CurrMapP, trans = "nl", smin = 0, smax = 255)
writeRaster(CurrMapP_natLog, filename=file.path(ConnDir,paste0('BC_P_1tile/CurrMapP_natLog.tif')), overwrite=TRUE)



#############

normalized_cum_currmap <- terra::rast(file.path(spatialOutDir,'ConnData',RunDir,'normalized_cum_currmap.tif'))
cum_currmap <- raster(file.path(RunDir,'cum_currmap.tif'))
flow_potential <- raster(file.path(RunDir,'flow_potential.tif'))
...
norm_cum_currSm <- terra::focal(normalized_cum_currmap, w=3, fun='mean')

#Classify normalized_cum_currmap
m <- c(0,0.2,1,
       0.2,0.7, 2,
       0.7,1.3,3,
       1.3,1.7,4,
...
norm_cum_curr<-classify(norm_cum_currSm,rclmat,right=TRUE)
#norm_cum_curr <- reclassify(normalized_cum_currmap, rclmat)

writeRaster(norm_cum_curr, filename=file.path(spatialOutDir,'norm_cum_curr3.tif'), overwrite=TRUE)


#Smooth
norm_cum_currSm <- terra::focal(norm_cum_curr, w=3, fun='mean')
...
#Contour experiment
x<-read_stars(file.path(ConnDir,RunDir,'normalized_cum_currmap.tif'))
currmapContour<-st_contour(x,contour_lines=TRUE, breaks=1:3)
plot(currmapContour[1], key.pos = 1, pal = sf.colors, lwd = 2, key.length = 0.8)
