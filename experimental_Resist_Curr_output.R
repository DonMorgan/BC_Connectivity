...
norm_cum_currW3 <- terra::focal(normalized_cum_currmap, w=3, fun='mean')
writeRaster(norm_cum_currW3, filename=file.path(spatialOutDir,'norm_cum_currW3.tif'), overwrite=TRUE)
...
norm_cum_curr3RC<-classify(norm_cum_currSm3,rclmat,right=TRUE)
#norm_cum_curr <- reclassify(normalized_cum_currmap, rclmat)
writeRaster(norm_cum_curr3RC, filename=file.path(spatialOutDir,'norm_cum_curr3RC.tif'), overwrite=TRUE)

#Resistance layer
Resist <- terra::rast(file.path(SpatialDir,'resistance_surface.tif'))

# Class into 5 categories
m <- c(0,1,1,
       1,4,2,
       4,50,3,
       50,80,4,
...
rclmat <- matrix(m, ncol=3, byrow=TRUE)
Resist_rc<-classify(Resist,rclmat,right=TRUE)
#norm_cum_curr <- reclassify(normalized_cum_currmap, rclmat)
writeRaster(Resist_rc, filename=file.path(spatialOutDir,'Resist_rc.tif'), overwrite=TRUE)
